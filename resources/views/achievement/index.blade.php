...
@section('content')
  <div class="p-8 max-w-7xl mx-auto">
    ...
    <div class="bg-gradient-to-br from-blue-600 to-purple-600 rounded-2xl p-10 mb-8 text-white shadow-lg">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
        <div
          class="bg-white bg-opacity-10 backdrop-blur-md rounded-xl p-6 flex items-center gap-4 border border-white border-opacity-20 transition hover:-translate-y-1 hover:bg-opacity-15">
    ...
    @if(session('success'))
      <div
        class="mb-6 flex items-center gap-4 rounded-xl border-l-4 border-green-500 bg-gradient-to-r from-green-100 to-green-200 p-5 text-green-800">
        <div class="text-2xl">
        ...
        <button onclick="this.parentElement.style.display='none'"
          class="ml-auto text-lg opacity-70 transition hover:opacity-100">
    ...
    @if(session('error'))
      <div
        class="mb-6 flex items-center gap-4 rounded-xl border-l-4 border-red-500 bg-gradient-to-r from-red-100 to-red-200 p-5 text-red-800">
        <div class="text-2xl">
    ...
    <!-- Achievements Table -->
    <div class="bg-white mb-8 rounded-2xl shadow-lg overflow-hidden">
        ...
        <div class="overflow-x-auto">
          <table class="w-full min-w-[1000px]">
            <thead class="bg-gray-50">
              <tr>
                <th
                  class="px-6 py-4 text-center text-xs font-semibold text-gray-700 uppercase tracking-wider border-b-2 border-gray-200">
                  No</th>
                <th
            ...
            <tbody class="divide-y divide-gray-200">
              @foreach ($achievements as $achievement)
                <tr class="hover:bg-gray-50 transition-colors achievement-row" data-level="{{ $achievement->level }}"
                  data-year="{{ $achievement->year }}" data-title="{{ strtolower($achievement->title) }}">
                  ...
                  <td class="px-6 py-5 align-middle">
                    ...
                    @endphp
                    <span
                      class="inline-block px-3 py-1 text-xs font-semibold rounded-full text-center min-w-[100px] {{ $badgeClass }}">
                      {{ $achievement->level }}
                    </span>
                  </td>
                  ...
                  <td class="px-6 py-5 align-middle text-center">
                    <span
                      class="inline-block px-3 py-1 bg-gray-100 rounded-md font-semibold text-gray-600 text-sm">{{ $achievement->year }}</span>
                  </td>

                  <td class="px-6 py-5 align-middle">
                    @if ($achievement->image)
                      <div class="relative w-20 h-20 rounded-lg overflow-hidden cursor-pointer group"
      ...
      @else
        ...
        <div class="text-center py-20 px-6">
          ...
          <a href="{{ route('admin.achievements.create') }}"
            class="inline-flex items-center gap-2 px-5 py-3 font-semibold text-white bg-gradient-to-br from-blue-600 to-purple-600 rounded-lg shadow-md hover:-translate-y-0.5 hover:shadow-lg transition-transform">
            <i class="fas fa-plus"></i>
      ...
      <div class="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
        <div class="flex items-center justify-between mb-6">
          <h3 class="text-xl font-bold text-gray-800 flex items-center gap-2">
            ...
            Sebaran Tingkat Prestasi
          </h3>
        </div>
        <div class="relative" style="height: 300px;">
          <canvas id="levelChartFull"></canvas>
        </div>
      </div>
      ...
      <div class="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
        <div class="flex items-center justify-between mb-6">
          <h3 class="text-xl font-bold text-gray-800 flex items-center gap-2">
            ...
            Tren Prestasi Tahunan
          </h3>
        </div>
        <div class="relative" style="height: 300px;">
          <canvas id="yearChartFull"></canvas>
        </div>
      </div>
    </div>
  ...
  <div id="imageModal" class="hidden fixed inset-0 bg-black bg-opacity-75 items-center justify-center p-4 z-[2000]">
    <div class="relative bg-white rounded-2xl shadow-2xl max-w-4xl max-h-[90vh] p-4">
      ...
      <button id="closeImageModal"
        class="absolute -top-4 -right-4 w-10 h-10 bg-gray-800 rounded-full text-white flex items-center justify-center cursor-pointer">
        <i class="fas fa-times"></i>
      </button>
    </div>
  </div>
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md transform transition-all">
      <div class="px-6 py-4 border-b">
        <h3 class="font-bold text-lg flex items-center gap-2"><i
            class="fas fa-exclamation-triangle text-red-500"></i>Konfirmasi Hapus</h3>
      </div>
      <div class="p-6">
        <p>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
      // Data dari AchievementController
      const levelLabels = {!! json_encode($levelStats->pluck('level')) !!};
      const levelData = {!! json_encode($levelStats->pluck('total')) !!};
      const yearLabels = {!! json_encode($yearStats->pluck('year')) !!};
      const yearData = {!! json_encode($yearStats->pluck('total')) !!};

      // 1. Level Chart (Doughnut)
      new Chart(document.getElementById('levelChartFull'), {
        type: 'doughnut',
        data: {
          labels: levelLabels,
          datasets: [{
      ...
      // 2. Year Chart (Bar)
      new Chart(document.getElementById('yearChartFull'), {
        type: 'bar',
        data: {
          labels: yearLabels,
          datasets: [{
            label: 'Jumlah Prestasi Per Tahun',
            data: yearData,
