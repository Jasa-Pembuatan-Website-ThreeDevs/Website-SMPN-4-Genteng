    ...
    <div class="bg-white shadow-md rounded-lg px-8 pt-6 pb-8 mb-4">
        <form action="{{ route('admin.ekstrakurikulers.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="number" name="student_count" id="student_count" min="0" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ old('student_count', 0) }}">
                @error('student_count')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            </div>
