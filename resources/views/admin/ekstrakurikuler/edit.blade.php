        ...
        <form action="{{ route('admin.ekstrakurikulers.update', $ekstrakurikuler->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="student_count" value="{{ old('student_count', $ekstrakurikuler->student_count) }}" 
                ...
                <div class="relative">
                    ...
                    <div class="border-2 border-dashed border-slate-300 rounded-xl p-8 text-center hover:border-blue-500 hover:bg-blue-50 transition">
                        <div class="flex flex-col items-center">
                            ...
                            <p class="text-slate-600 font-medium">Drag & drop atau klik untuk ganti gambar</p>
                            <p class="text-slate-500 text-sm mt-1">Biarkan kosong jika tidak ingin mengubah gambar</p>
                        </div>
                    </div>
                </div>

                <div id="imagePreview" class="{{ $ekstrakurikuler->image ? '' : 'hidden' }} mt-4">
                    <p class="text-sm font-medium text-slate-700 mb-2">Preview Gambar:</p>
                    <div class="bg-slate-100 rounded-xl p-4">
                        <img id="previewImg" src="{{ $ekstrakurikuler->image ? asset('storage/' . $ekstrakurikuler->image) : '' }}" alt="Preview" class="max-h-64 mx-auto rounded">
...
<script>
    ...
    removeImageBtn.addEventListener('click', function(e) {
        ...
        @if($ekstrakurikuler->image)
            previewImg.src = "{{ asset('storage/' . $ekstrakurikuler->image) }}";
        @else
